...
export default class NewEvent extends Component {
  ...
  render() {
    ...
    <TextField
      ref='InfoInput'
      label='Information'
      baseColor={colors.newEventInfoInputBase}
  ...
  submitEvent() {
    console.log('New event submited!')
    ...
    if(this.state.title != '' || this.state.info != ''){
      ...
      this.setState({ data: newData}, () => {
        ...
        AsyncStorage.setItem("data", JSON.stringify(this.state.data))
          .then((data) => {
            console.log('Successfully stored a new Event to AsyncStorage!');
            this.props.navigation.goBack();
          })
          .catch(error => console.log('Error saving new event Data to AsyncStorage: ' + error));
      });
    } else {
      if(this.state.title == ''){
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 10,
    backgroundColor: colors.newEventBackground,
  },
  submitButton: {
    fontSize: 20,
